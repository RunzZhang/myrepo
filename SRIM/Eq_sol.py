...
class E_loss_solve():
    def __init__(self):
        ...
        self.LSS_modify = 0.8562 #0.552 - 0.8562   mid 0.716 set it as 1 if it ioriginal
    ...
    def E_loss_t_fun_ODE_new_D(self, t, y):
        ...
        ep = self.C_tf*0.5* (y*0.5/13.6)/(self.Z_tp**2*self.Z**(0.5))
        # part_a = - np.log(1+self.a * ep) / (2 * (ep + self.b * (ep) ** self.c) + self.d * (ep) ** 0.5)* self.factor *self.test_TarDen*10**8
        part_a = - self.LSS_modify*np.log(1 + self.a * ep) / (2 * (ep + self.b * (ep) ** self.c) + self.d * (
            ep) ** 0.5) /self.LSS_factor
        # part_a = - self.a * ep/ (
        #             2 * (ep + self.b * (ep) ** self.c) + self.d * (ep) ** 0.5) * self.factor * self.Tar_Den * 10 ** 8
        part_b = - (y) ** 0.5 * self.total_k
        part_c = (y) ** 0.5 * np.sqrt(2 * self.ev / self.mass) * (1/self.T_factor) # change dE/dx to dE/dt
    ...
    def E_loss_total_t_fun_ODE_posttest(self):
        ...
        plt.plot(t_fs, sol_y, linewidth = 4)
        Dy_list = []
