    ...
    def source_uncertainty(self, uncertainty):
        ...
        for i in range(len(x_bins)):
            ...
            for j in range(len(y_bins_low)-1):
                ...
                if y_bins_low[i] > y_bins[j] and j ==0 :
                    x_bins_low.append(0)
                elif y_bins_low[i] < y_bins[j] and j ==len(y_bins_low)-1:
                    x_bins_low.append(2*x_bins[i])
                elif y_bins_low[i]<= y_bins[j] and y_bins[i] > y_bins[j+1]:
                    x_bins_low.append(x_bins[j])
            for k in range(len(y_bins_high)-1):

                if y_bins_high[i] > y_bins[k] and k == 0 :
                    x_bins_high.append(0)
                elif y_bins_high[i] < y_bins[k+1] and k == len(y_bins_high)-1:
                    print(i,k)
