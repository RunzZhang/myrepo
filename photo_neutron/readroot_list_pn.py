...
class ReadRoot():
    def __init__(self):
        ...
        self.base_path2 = "/data/runzezhang/result/TN_sims_D/chunked_root_files_pn_outside_1E7/"
    ...
    def Huge_NR(self):
        ...
        self.single_scattering_wo_ncapER_wt_NR= self.single_scattering_wo_ncap_wt_NR[self.single_scattering_wo_ncap_wt_NR["Event"].isin(low_none_ER)]
    ...
    def PN_spectrum(self):
        ...
        self.df_ini = self.df[
            ...
            ['Event', 'Volume', 'Track ID', "X/mm",'Parent ID',"Step ID","PreKinetic/MeV"]]
        self.df_ke = self.df[
        ...
        print("ini", self.df_ini.head(20))
        print("ke",self.df_ke.head(20))
        ini_list = self.df_ini["PreKinetic/MeV"].to_list()
        ini_x_list = self.df_ini["X/mm"].to_list()
        ke_list = self.df_ke["PreKinetic/MeV"].to_list()
        with open(self.ini_path, 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(ini_list)

        with open(self.ar_ke_path, 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(ke_list)

        with open(self.ini_x_path, 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(ini_x_list)
    ...
    def find_gamma_e(self):
        ...
        print("max", max(p_observed), "\n", "min", min(p_observed))
        # plt.hist(self.electron_recoiled_list, bins=100)
        with open(self.base_path+"/photon_captureout.csv", 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(p_observed)
        plt.hist(p_observed, bins=100)
        plt.xlabel("Obeserved Photon per Event")
    ...
    def single_n_find_gamma_e_loop(self):
        self.df_gamma_rw = pd.read_csv(self.base_path2 + "dmx_single_n_gamma_PN_neutron_list_loop.csv")
        print(self.df_gamma_rw[["PreKinetic/MeV"]].head(20))
        # we need to do severalthings:
        # gamma only in LAr or CF4
        # in 1 event number, only the first series of gammas, avoiding over-countting
        ...
        print("max", max(p_observed), "\n", "min", min(p_observed))
        # plt.hist(self.electron_recoiled_list, bins=100)
        with open(self.base_path2+"/photon_capture_n_sing_scatterg_PN.csv", 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerow(p_observed)
        plt.hist(p_observed, bins=100)
        plt.xlabel("Obeserved Photon per Event")
