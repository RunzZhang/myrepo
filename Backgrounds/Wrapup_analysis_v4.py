...
class integrated_analysis():
    ...
    def gamma_rejection_plot_PSN_v2(self, rate_cut=False, radi_source="Cs", plot_fitting=True):
        ...
        self.df_Cs_119_plot = self.concat_PT_condition(self.df_Cs_119_plot, upperlimit=True, keep=False)
    ...
    def concat_PT_condition(self, df, upperlimit=True, keep=True):
        ...
        if upperlimit:
            ...
            print("PT concat upperlimit faLse", df_combined[["Clean Rate [mHz]","Clean Rate Sigma [mHz]"]] )
            cond = (df_combined["Clean Rate [mHz]"] > 0) & (
                    (df_combined["Clean Rate [mHz]"] - df_combined["Clean Rate Sigma [mHz]"]) > 0
            )
