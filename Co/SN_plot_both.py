...
class SN():
    def __init__(self):
        ...
        self.base_path2 = "/data/runzezhang/result/GR_sims/chunked_root_files_Co_5E6/"  # for gamma path
...
# main funtion we use
    ...
    def write_sims_results(self, plot=True):
        ...
        # hist_array[0] = np.histogram(ER_Ar, bins=100, range=(0, 1200))
        hist_array_primary[0] = np.histogram(ER_Ar_primary, bins=350, range=(0, 1400))
        ...
        hist_array_all = [None]
        # hist_array[0] = np.histogram(ER_Ar, bins=100, range=(0, 1200))
        hist_array_all[0] = np.histogram(ER_Ar_all, bins=350, range=(0, 1400))
    ...
    def write_sims_results(self, plot= True):
        ...
        hist_array_primary = [None]
        # hist_array[0] = np.histogram(ER_Ar, bins=100, range=(0, 1200))
        hist_array_primary[0] = np.histogram(ER_Ar_primary, bins=14000, range=(0, 1400))
        # hist_array_primary[0] = np.histogram(ER_Ar_primary, bins=300, range=(0, 1200))
        # 12 keV -> 1 Setiz threshold there is no change for gamma rejection
        # we need 0.1 keV, and this gives us 4800 bins
