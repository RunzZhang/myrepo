...
class ReadRoot:
    def __init__(self, doped = False, source ="Cs"):
        ...
        self.source = source
        self.filepath = self.base_path + f"dmx_{self.source}_5E6_ER.root"
    ...
    def chunk_and_write_root(self, start_chunk_cum = 0,num_chunks=20, output_dir=None):
        if output_dir is None:
            output_dir = os.path.join(self.base_path, f"chunked_root_files_{self.source}_5E6_ER")
        ...
        with uproot.open(self.filepath) as file:
            ...
            for arrays in tree.iterate(expressions=self.selected_columns, library="pd", entry_start=0,
                ...
                chunk_num += 1
                output_filename = os.path.join(output_dir, f"dmx_{self.source}_{chunk_num}.root")
...
if __name__ == "__main__":
    reader = ReadRoot(doped=False, source= "Cs")
    # reader = ReadRoot(doped=True, source="Cs")
