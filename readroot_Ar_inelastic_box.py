...
class ReadRoot():
    ...
    def single_run(self):
        ...
        self.inelastic_gamma_path = self.base_path + "inelastic_gamma_energies.csv"
    ...
    def Huge_scatter_spectrum(self):# photon generated by the scattering instead other process
        print("Process",self.df["Process"].unique())
    ...
    def plot_neutron_inelastic_spectrum(self):
        ...
        axs[0].hist(self.noise5_raw_list, bins=np.logspace(-5, 7, 500))
        axs[0].set_title("Neutron Initial energy; total counts " + str(len(self.noise5_raw_list)))
        axs[0].set_xscale('log')
        axs[0].set_yscale('log', nonpositive="clip")
        axs[0].set_xlim(1e-3, 1e7)
        axs[0].set_xlabel("Energy/eV")
        axs[0].set_ylabel("Counts")

        axs[1].hist(self.noise4_raw_list, bins=500)
        axs[1].set_title("Inelastic Gamma energy; total counts " + str(len(self.noise4_raw_list)))
        # axs[1].set_xscale('log')
        # axs[1].set_yscale('log')
        axs[1].set_xlticks(range(0,10,1))
        axs[1].set_xlim(0, 10)
