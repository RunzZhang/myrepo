...
################################################
# flist = ["../Co60/JAEA.txt","../Sb124/JAEA.txt","../Test_Th228/JAEA.txt"]
...
def analyze(file, T, sigLow, sigUp, N=1 * 10 ** 8, Activity=100, time=100):
    ...
    for nw in range(Nwalkers):
        ...
        for ni in range(NitersRough):
            ...
            energiesi[:], nuisancei[:], modei[:] = stepper(
                energiesc, nuisancec, modec, step * step_shift * RoughFact, n_step * step_shift, m_step * step_shift,
                max=2000
            )
            if Nuisance == False:
