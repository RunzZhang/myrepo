...
class multi_MC():
    def __init__(self):
        ...
        self.save_path = self.fileprefix
        """ 
    ...
    def plot_error(self, mean, high, low, th, siglow, sighi):
        x = range(0, max([1200,th+3*sighi]), 1)
        y = []
        for i in x:
    ...
    def analyze(self,file, T, sigLow, sigUp, N=2 * 10 ** 9, Activity=100, time=100):
        ...
        # AmLi source
        t = 2E5 / 1.18  # updated 2E5 events capture rate LZ bare is 4.17 , the capture rate is 0.94
        # pn source with lead
        t = 2E5/1.97 # updated 2E5 events capture rate ,pn capture rate after tagging is 1.97
        # veto analyze
